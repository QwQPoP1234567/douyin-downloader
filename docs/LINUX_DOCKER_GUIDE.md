...
| 扫描并发 | `1` | 浏览器只有一把扫描页锁，调大只会排队 |
| 应用内存上限 | `5g` | Chromium 比较吃内存，建议不低于 `2g` |
| MySQL 内存上限 | `2g` | 与 1G InnoDB 缓冲池匹配 |
| MySQL 缓冲池 | `1G` | 内存紧张时可降到 `256M` |
| 容器临时目录大小 | `512m` | 内存盘，计入应用内存上限 |
| 单个容器日志上限 | `10m` | 防止容器日志吃光系统卷 |
| noVNC | 启用 | 首次登录必须启用，后续可按需关闭 |

向导不提供 CPU 上限：群晖的 Container Manager 不认 compose 的 `cpus` 参数，写了会直接报错。
...
内存紧张的 NAS 可以把应用内存上限降到 `2g`、MySQL 内存上限降到 `1g`、缓冲池降到 `256M`，并保持下载并发和扫描并发都为 1。
